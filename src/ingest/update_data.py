...
# IESO posts tomorrow's DAM at ~12:35 market time (EST, no DST): the report's Last-Modified was
# 17:34-17:39 UTC every day 15-22/09/2026. 13:00 leaves GridStatus time to ingest it.
PUBLISH_TIME_EST = pd.Timedelta(hours=13)


def dam_is_current(now=None):
    """True when the file already holds every DAM day IESO has published so far: today's, plus
    tomorrow's once the ~12:35 EST publication has passed. DAM prices are final once posted, so
